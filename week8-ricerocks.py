...
Descprition: Implementation of RiceRocks (Asteroids)
...
TURN_FACTOR = 0.05
THRUST_FACTOR = 0.1 # acceleration
FRICTION_FACTOR = 0.99 # de-acceleration factor
MISSILE_SPEED_FACTOR = 6
MAX_ROCKS = 12
...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust:
            ...
            self.vel[0] += acc[0] * THRUST_FACTOR
            self.vel[1] += acc[1] * THRUST_FACTOR
    ...
    def shoot(self):
        ...
        missile_vel = [self.vel[0] + MISSILE_SPEED_FACTOR * forward[0],
                       self.vel[1] + MISSILE_SPEED_FACTOR * forward[1]]
...
class Sprite:
    ...
    def collide(self, other_object, offset = 0):
        collision = False
        if dist(self.pos, other_object.get_position()) < self.radius + other_object.get_radius() + offset:
            collision = True
        return collision
# END Sprite class
...
def rock_spawner():
    if len(rock_group) < MAX_ROCKS and started:
        ...
        while add_rock:
            ...
            rock_vel = [random.random() * .6 - .3 * (score/4) * random.choice([-1, 1]),
                        random.random() * .6 - .3 * (score/4) * random.choice([-1, 1])]
            rock_avel = random.random() * .2 - .1
            new_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
            if not new_rock.collide(my_ship, 20):
                rock_group.add(new_rock)
                add_rock = False

# helper function to update and (call) draw on sprite group       
def process_sprite_group(sprite_group, canvas):
    ...
    for sprite in sprite_group:
        ...
        sprite.draw(canvas)
    sprite_group.difference_update(remove_group)
        
# helper function to detect collisions on a group of sprites
def group_collide(sprite_group, other_object):
    ...
    sprite_group.difference_update(collision_group)    
    return collision
...
def group_group_collide(first_group, second_group):
    ...
    for obj in first_group:
        if group_collide(second_group, obj):
            elements_collide += 1
            remove_group.add(obj)
    first_group.difference_update(remove_group)
